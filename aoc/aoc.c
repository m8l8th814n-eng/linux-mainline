...
/*
 * dma-buf and vmalloc used to arrive transitively through the vendor's
 * <samsung/samsung-dma-heap.h>. Include them directly now that it is gone.
 */
...
static int find_gsa_device(struct aoc_prvdata *prvdata)
{
	...
	/*
	 * GSA is only used to authenticate a signed firmware image, and that
	 * path is taken solely when "gsa-enabled" is set. There is no GSA
	 * driver in mainline, so treat a missing gsa-device as fine unless
	 * gsa-enabled asks for it; the unsigned boot path needs no GSA.
	 */
	prvdata->gsa_dev = NULL;

	np = of_parse_phandle(prvdata->dev->of_node, "gsa-device", 0);
	if (!np) {
		if (of_property_read_bool(prvdata->dev->of_node, "gsa-enabled")) {
			dev_err(prvdata->dev,
				"gsa-enabled set but no gsa-device phandle\n");
			return -ENODEV;
		}
		dev_info(prvdata->dev,
			 "no gsa-device, using unsigned firmware boot path\n");
		return 0;
	}
...
static void aoc_pheap_map(struct aoc_dma_buffer *buffer, void *ctx, bool should_map)
{
	struct device *dev = ctx;
	struct aoc_prvdata *prvdata = dev_get_drvdata(dev);
...
static void aoc_pheap_alloc_cb(struct aoc_dma_buffer *buffer, void *ctx)
{
	aoc_pheap_map(buffer, ctx, true);
}

static void aoc_pheap_free_cb(struct aoc_dma_buffer *buffer, void *ctx)
{
...
long aoc_unlocked_ioctl_handle_ion_fd(unsigned int cmd, unsigned long arg)
{
	...
	struct aoc_dma_buffer *dma_heap_buf;
	long ret = -EINVAL;
...
static int aoc_platform_probe(struct platform_device *pdev)
{
	...
	/*
	 * The iommus phandle is optional here. Mainline has no SysMMU nodes
	 * for gs101 at all, and this is only used to pick up the two IOMMU
	 * fault interrupt numbers -- no mapping is set up through it. Leaving
	 * the irqs at zero disables the fault reporting; the enable/disable
	 * sites in aoc_v1.c skip them when unset.
	 */
	...
	if (iommu_node) {
		...
		dev_info(dev, "no iommus phandle, iommu fault reporting disabled\n");
		prvdata->iommu_nonsecure_irq = 0;
		prvdata->iommu_secure_irq = 0;
	}
	...
	if (!prvdata->domain) {
		/*
		 * Both this and the SysMMU driver are modules, so probe order
		 * is whatever udev happens to pick. The original -EIO here
		 * made a lost race permanent; defer instead, so registering
		 * the SysMMU driver retriggers this probe.
		 */
		dev_info(dev, "no iommu domain yet, deferring\n");
		rc = -EPROBE_DEFER;
		goto err_find_iommu;
	}
	...
	/*
	 * WIP: non-fatal on purpose.
	 *
	 * ion_physical_heap.c was reimplemented on the plain dma-heap API and
	 * still hands out buffers as struct page, which do not exist for the
	 * no-map aoc_reserve region. The heaps are only needed by userspace
	 * clients (sensors, playback, capture), not to boot AoC itself, so
	 * carry on without them until that is reworked to use raw physical
	 * addresses and remap_pfn_range().
	 */
	if (!aoc_create_dma_buf_heaps(prvdata))
		dev_warn(dev, "no dma_buf heaps; userspace buffer sharing unavailable\n");
