...
static int aocc_probe(struct aoc_service_dev *dev)
{
	...
	if (strcmp(dev_name(&dev->dev), "usf_sh_mem_doorbell") != 0) {
		...
		if (IS_ERR(prvdata->demux_task)) {
			...
			/*
			 * sched_setscheduler() is not exported to modules in
			 * mainline. sched_set_fifo_low() is the exported
			 * equivalent and is the closest match to the priority
			 * 10 this used to ask for.
			 *
			 * Note this used to run before the IS_ERR() check,
			 * i.e. on an ERR_PTR when kthread_run() failed.
			 */
			sched_set_fifo_low(prvdata->demux_task);
		}
	}
