		...
		struct aoc_prvdata *prvdata)
{
	...
	/*
	 * The secure interrupt is optional here. Reaching it needs the secure
	 * SysMMU path, which talks to trusted firmware over SMC and is not
	 * available on this platform, so the device tree only declares the
	 * non-secure one. Leave the number at zero; the enable/disable sites
	 * above skip it when unset.
	 */
	ret = of_irq_get(iommu_node, 1);
	if (ret < 0) {
		dev_info(dev, "no iommu secure irq (%d), continuing without it\n",
			 ret);
...
void configure_crash_interrupts(struct aoc_prvdata *prvdata, bool enable)
{
	if (prvdata->first_fw_load) {
		...
		/* Zero when there is no iommu node; see platform_probe(). */
		if (prvdata->iommu_nonsecure_irq)
			enable_irq(prvdata->iommu_nonsecure_irq);
		if (prvdata->iommu_secure_irq)
