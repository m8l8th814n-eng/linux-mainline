...
#define CIDINFO_DATA "/var/log/.cid.info"
#endif /* PLATFORM_SLP */
...
dhd_read_cis(dhd_pub_t *dhdp)
{
	...
	strscpy(g_cis_buf, "cisdump", buf_size);
...
dhd_check_module_mac(dhd_pub_t *dhdp)
{
	...
	if (!g_cis_buf) {
		...
		if (dhd_read_file(filepath_efs, otp_mac_buf, sizeof(otp_mac_buf) - 1) < 0) {
			...
			strscpy(eabuf, otp_mac_buf, sizeof(eabuf));
...
#else
	strscpy(cidinfostr, cid_info, MAX_VNAME_LEN);
#endif /* DHD_EXPORT_CNTL_FILE */
...
#else
	strscpy(vname, cidinfostr, MAX_VNAME_LEN);
#endif /* DHD_EXPORT_CNTL_FILE */
...
#else
	strscpy(vid, cidinfostr, MAX_VID_LEN);
#endif /* DHD_EXPORT_CNTL_FILE */
