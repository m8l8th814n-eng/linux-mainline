...
#if !defined(CONFIG_SOC_GOOGLE)
/*
 * Mainline build: the vendor Exynos PCIe RC driver (exynos-pci-ctrl.h and the
 * exynos_pcie_* API) is not present. The chip is brought up by the mainline
 * pcie-gs101 controller and an always-on regulator, so these become no-ops.
 */
#ifndef MAX_HW_INFO_LEN
#define MAX_HW_INFO_LEN 20
#endif
#define EXYNOS_PCIE_EVENT_LINKDOWN	0x1
#define EXYNOS_PCIE_EVENT_CPL_TIMEOUT	0x2
#define EXYNOS_PCIE_TRIGGER_CALLBACK	0
#define PCIE_L1SS_CTRL_WIFI		0
/* IRQ affinity: gs101 topology cpu0-3 A55 (little), cpu4-5 A76, cpu6-7 X1 (big) */
#define IRQ_AFFINITY_BIG_CORE		6
#define IRQ_AFFINITY_SMALL_CORE		0
#define AFFINITY_UPDATE_MIN_PERIOD_SEC	1
#define RESCHED_STREAK_MAX_HIGH		100
#define RESCHED_STREAK_MAX_LOW		10
#define RESCHED_CNT_CHECK_PERIOD_SEC	1
...
static inline int exynos_pcie_pm_resume(int ch) { return 0; }
static inline void exynos_pcie_pm_suspend(int ch) { }
static inline int exynos_pcie_l1_exit(int ch) { return 0; }
static inline void exynos_pcie_register_dump(int ch) { }
static inline int exynos_pcie_rc_l1ss_ctrl(int a, int b, int c) { return 0; }
static inline int exynos_pcie_register_event(struct exynos_pcie_register_event *e) { return 0; }
static inline int exynos_pcie_deregister_event(struct exynos_pcie_register_event *e) { return 0; }
#endif /* !CONFIG_SOC_GOOGLE */
...
dhd_wifi_init_gpio(void)
{
	...
	if (!root_node) {
		/*
		 * No "android,bcmdhd_wlan" node on mainline gs101. WLAN power comes
		 * from an always-on regulator and the chip is already on the PCIe
		 * bus, so there is no WL_REG_ON GPIO to drive. Skip GPIO setup and
		 * mark the pin invalid; dhd_wlan_power() becomes a no-op.
		 */
		DHD_ERROR(("no BRCM WLAN DT node; assuming always-on regulator, "
			"skipping GPIO setup\n"));
		wlan_reg_on = -1;
		return 0;
	}
...
dhd_wlan_power(int onoff)
{
	...
	if (!gpio_is_valid(wlan_reg_on)) {
		/* No WL_REG_ON GPIO (always-on regulator on mainline): nothing to do. */
		return 0;
	}
