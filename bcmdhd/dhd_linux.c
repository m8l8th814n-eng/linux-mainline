...
/* Error bits */
/* 0644 so the level can be changed at runtime via
 * /sys/module/bcmdhd/parameters/dhd_msg_level; with perm 0 no sysfs entry is
 * created at all and the level can only be set at module load time.
 */
	...
	uint8 *mac, uint8 bssidx, const char *dngl_name, int if_state)
{
	...
	if (if_state == NDEV_STATE_FW_IF_CREATED) {
		...
		if (dngl_name) {
			strscpy_pad(ifp->dngl_name, dngl_name, IFNAMSIZ);
		} else if (ndev->name[0] != '\0') {
			strscpy_pad(ifp->dngl_name, ndev->name, IFNAMSIZ);
		}
		if (mac != NULL) {
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	/* Enable oob at firmware (SDIO only; PCIe w/o OOB uses in-band MSI) */
...
/* xxx andrey tmp fix for dk8000 build error  */
	...
	if (ret < 0)
		...
		strscpy_pad(fw_version, buf, FW_VER_STR_LEN);
	...
	if (ret < 0)
		...
		if ((ver_temp_buf = bcmstrstr(ptr, "Customization:")) == NULL) {
			...
			strscpy_pad(clm_ver_temp, clm_version, strlen(clm_version));
...
dhd_os_runtimepm_timer(void *bus, uint tick)
{
	...
	if (tick) {
		...
		if (dhd->rpm_timer_valid == TRUE) {
			dhd->rpm_timer_valid = FALSE;
			DHD_GENERAL_UNLOCK(pub, flags);
			timer_delete_sync(&dhd->rpm_timer);
...
write_dump_to_file(dhd_pub_t *dhd, uint8 *buf, int size, char *fname)
{
	...
	file_mode = O_CREAT | O_WRONLY | O_SYNC;
	{
		...
		if (IS_ERR(fp) || (fp == NULL)) {
			...
			snprintf(memdump_path, sizeof(memdump_path), "%s%s_%s_" "%s",
				"/var/log/", fname, memdump_type,  dhd->debug_dump_time_str);
		} else {
...
dhd_dump_file_manage_idx(dhd_dump_file_manage_t *fm_ptr, char *fname)
{
	...
	if (strlen(fm_ptr->elems[fm_idx].type_name) == 0) {
		strscpy_pad(fm_ptr->elems[fm_idx].type_name, fname, DHD_DUMP_TYPE_NAME_SIZE);
...
dhd_dump_file_manage_enqueue(dhd_pub_t *dhd, char *dump_path, char *fname)
{
	...
	/* save dump file path */
	strscpy_pad(elem->file_path[fp_idx], dump_path, DHD_DUMP_FILE_PATH_SIZE);
