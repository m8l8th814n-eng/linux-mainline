		...
		if (ifp && ifp->net->qdisc) {
			...
			if (dev_ingress_queue(ifp->net)) {
				...
				if (qdisc != NULL && (qdisc->flags & TCQ_F_INGRESS)) {
					...
					if (0) /* miniq_ingress removed from net_device in 6.6+ */
...
dhd_rx_pktpool_deinit(dhd_info_t *dhd)
{
	 ...
	 * Guard against an uninitialized queue (skb_queue_head_init() never ran,
	 * e.g. attach failed early): a zeroed skb_q has next == NULL and
	 * skb_queue_purge() would dereference it.
	 */
	if (rx_pool->skb_q.next) {
