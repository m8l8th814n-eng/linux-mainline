...
dhdpcie_start_host_dev(dhd_bus_t *bus)
{
	...
	/*
	 * Mainline gs101 (pcie-gs101) has no vendor RC power-on hook to retrain
	 * the link after the dongle is reset -- exynos_pcie_pm_resume() is a
	 * no-op here. If the endpoint has fallen off the bus (config space reads
	 * all-ones), issue a secondary bus reset on the upstream root port. That
	 * resets the endpoint and lets the DesignWare root complex retrain the
	 * link (LTSSM), which is what exynos_pcie_poweron() does on Android.
	 */
	{
		struct pci_dev *rootport = pci_upstream_bridge(bus->dev);
		uint32 vid = 0xffffffff;

		pci_read_config_dword(bus->dev, PCI_CFG_VID, &vid);
		if (rootport && vid == 0xffffffff) {
			uint16 bctl = 0;
			DHD_ERROR(("%s: EP off bus (vid=0x%x); secondary bus reset on RC\n",
				__FUNCTION__, vid));
			pci_read_config_word(rootport, PCI_BRIDGE_CONTROL, &bctl);
			pci_write_config_word(rootport, PCI_BRIDGE_CONTROL,
				bctl | PCI_BRIDGE_CTL_BUS_RESET);
			OSL_SLEEP(2);
			pci_write_config_word(rootport, PCI_BRIDGE_CONTROL, bctl);
			/* wait for the link to retrain and the EP to re-appear */
			OSL_SLEEP(100);
			pci_read_config_dword(bus->dev, PCI_CFG_VID, &vid);
			DHD_ERROR(("%s: after secondary bus reset vid=0x%x\n",
				__FUNCTION__, vid));
		}
	}
#endif /* !CONFIG_ARCH_MSM */
...
#endif /* SUPPORT_LINKDOWN_RECOVERY */
#endif /* CONFIG_ARCH_MSM */
#if 0 /* CONFIG_PCI_TEGRA: not applicable on gs101 */
