		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (cs35l41->halo_booted == false)
			...
			cs35l41->dsp.cs_dsp.booted = true;
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (cs35l41->dsp.preloaded) {
		...
		snd_soc_dapm_disable_pin(dapm, "DSP1 Preload");
		snd_soc_dapm_sync(dapm);
		usleep_range(4000, 5000);
		snd_soc_dapm_force_enable_pin(dapm, "DSP1 Preload");
		snd_soc_dapm_sync(dapm);
				  ...
				  struct cs35l41_private *cs35l41)
{
	...
	/*
	 * Feed cirrus,subsystem-id into dsp.system_name. wm_adsp only appends
	 * the system name and the ASoC component name-prefix to the firmware
	 * filename when dsp.system_name is set; leaving it NULL makes wm_adsp
	 * use the prefix-less legacy name, so both amps request
	 * cs35l41-dsp1-spk-prot.bin instead of their per-speaker
	 * cs35l41-dsp1-spk-prot-oriole{,-r}.bin.
	 */
	{
		const char *sub;

		if (!of_property_read_string(np, "cirrus,subsystem-id", &sub))
			cs35l41->dsp.system_name =
				devm_kstrdup_const(cs35l41->dev, sub, GFP_KERNEL);
	}
