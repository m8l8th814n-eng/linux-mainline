...
static int bcm_spi_probe(struct spi_device *spi)
{
	...
	/*
	 * driver_override no longer lives in struct platform_device: it moved
	 * into struct device behind a lock, and must be set through the helper
	 * rather than assigned. Same change the AoC driver needed.
	 */
	if (device_set_driver_override(&priv->sscd_dev.dev, SSCD_NAME))
		dev_err(&spi->dev, "unable to set sscd driver override\n");
...
static const struct spi_device_id bcm_spi_id[] = {
	/*
	 * The SPI core strips the vendor prefix from the compatible and looks
	 * the remainder up here, so "ssp-spi,bcm4775" wants a "bcm4775" entry.
	 * Without it every bind logs "has no spi_device_id for ssp-spi,bcm4775".
	 */
	{"bcm4775", 0},
	{"ssp-spi", 0},
	{}
...
/*
 * Without this the module gets no of: alias and udev never autoloads it when
 * the device tree node appears. Android loaded it by name from init, so the
 * vendor never needed one.
 */
