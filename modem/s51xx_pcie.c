...
static int s51xx_pcie_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	/*
	 * s51xx_pci_id_tbl matches every Samsung device, and the DesignWare
	 * root port is one -- 144d:a5a5. Binding to it walks into bus->self,
	 * which is NULL on a root bus, and the BAR fixups below are meaningless
	 * on a bridge anyway. The modem is an endpoint.
	 */
	if (pci_is_bridge(pdev) || !bus_self)
		return -ENODEV;
