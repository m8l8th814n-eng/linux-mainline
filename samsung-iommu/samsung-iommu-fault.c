...
/*
 * enum iommu_fault_reason and struct iommu_fault_event went away with the old
 * iommu_report_device_fault() interface, so these are local. They only feed
 * the fault message text now; nothing is reported to the iommu core. See
 * samsung_sysmmu_irq_thread(). SYSMMU_FAULT_* is taken by the hardware codes.
 */
enum sysmmu_fault_reason {
	SYSMMU_FR_WALK_EABT,
	SYSMMU_FR_PTE_FETCH,
	SYSMMU_FR_ACCESS,
	SYSMMU_FR_UNKNOWN,
...
static unsigned int __maybe_unused sysmmu_fault_type[SYSMMU_FAULTS_NUM] = {
	SYSMMU_FR_WALK_EABT,
	SYSMMU_FR_PTE_FETCH,
	SYSMMU_FR_ACCESS,
	SYSMMU_FR_UNKNOWN,
...
/*
 * samsung_sysmmu_fault_notifier() lived here. It walked the iommu group
 * and called iommu_report_device_fault() for the matching client. Both
 * that function and struct iommu_fault_event are gone upstream, and
 * nothing consumed the notifications, so it was dropped rather than
 * rewritten against the newer page-fault interface.
 */
...
irqreturn_t samsung_sysmmu_irq_thread(int irq, void *dev_id)
{
	...
	/*
	 * The vendor driver reported the fault to the iommu core here through
	 * iommu_report_device_fault() and struct iommu_fault_event, cleared
	 * the interrupt only if a client answered -EAGAIN, and panicked
	 * otherwise. That interface no longer exists upstream.
	 *
	 * Take the clear-and-continue path unconditionally instead: the fault
	 * is printed below and the interrupt acknowledged, so a stray DMA no
	 * longer takes the phone down. Nothing consumed fault notifications on
