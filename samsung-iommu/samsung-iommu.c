...
/*
 * domain_alloc(type) was split upstream into per-kind callbacks, and
 * pgsize_bitmap moved from struct iommu_ops into struct iommu_domain.
 * Only paging domains are wired up -- that is what AoC uses. An IDENTITY
 * domain would need its own domain_alloc_identity().
 */
...
static void __maybe_unused samsung_sysmmu_detach_dev(struct iommu_domain *dom,
				      struct device *dev)
{
	struct sysmmu_clientdata *client;
	struct samsung_sysmmu_domain *domain = to_sysmmu_domain(dom);
...
static size_t __maybe_unused samsung_sysmmu_unmap(struct iommu_domain *dom,
					 ...
					 unsigned long iova, size_t size)
{
	...
	while (iova < end) {
		...
		/*
		 * Only descend into a second-level table if one is actually
		 * there. The original code assumed every entry in the synced
		 * range was either a section or a page table; an empty entry
		 * makes lv2table_base() zero and page_entry() dereference
		 * phys_to_virt(0), which faults at translation level 0.
		 */
		if (!lv1ent_page(sent))
			goto next_section;
...
/*
 * map()/unmap() were dropped from iommu_domain_ops in favour of the _pages
 * variants. unmap_pages() already exists; this wraps the existing map().
 */
static int samsung_sysmmu_map_pages(struct iommu_domain *dom, unsigned long iova,
				    ...
				    int prot, gfp_t gfp, size_t *mapped)
{
	...
	/*
	 * samsung_sysmmu_map() installs exactly one entry, of a size that must
	 * be in pgsize_bitmap: it either writes a section descriptor or a
	 * single lv2 page. Handing it pgsize * pgcount would make it treat the
	 * whole span as one page and corrupt the page table, so loop instead
	 * and report how far we got -- the core retries the remainder.
	 */
	while (pgcount--) {
		ret = samsung_sysmmu_map(dom, iova + done, paddr + done,
					 pgsize, prot, gfp);
		if (ret)
			break;
		done += pgsize;
...
static void samsung_sysmmu_release_device(struct device *dev)
{
	...
	/* iommu_fwspec_free() is gone; the core releases it. */
}
...
static struct iommu_group *samsung_sysmmu_device_group(struct device *dev)
{
	...
	if (need_unmanaged_domain) {
		...
		/* iommu_domain_alloc(bus) is gone; allocate against the device. */
...
/*
 * PASID and device-feature support, compiled out.
 *
 * set_dev_pasid() gained a trailing "old domain" argument and
 * remove_dev_pasid() was built on iommu_get_domain_for_dev_pasid(), which no
 * longer exists. dev_has_feat()/dev_enable_feat()/dev_disable_feat() were
 * removed from struct iommu_ops outright.
 *
 * Nothing on gs101 uses any of it -- AoC attaches a single plain domain -- so
 * the code is kept intact but disconnected rather than rewritten blind.
 */
#if 0
...
#endif /* PASID and device-feature support disabled */
...
static struct iommu_ops samsung_sysmmu_ops = {
	...
	.owner						= THIS_MODULE,
	/*
	 * Gone from struct iommu_ops upstream: pgsize_bitmap (now per domain),
	 * dev_enable_feat/dev_disable_feat, and the PASID callbacks. See the
	 * disabled block further up.
	 */
...
/* Android loads this by name at boot; udev needs a modalias to autoload it. */
